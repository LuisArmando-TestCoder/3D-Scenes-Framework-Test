...
export default {
  spaceModule: {
      ...
      return consulters.getProceduralGroup([
        {
          ...
          getIntersectionMesh([x, y, z]) {
            ...
            satellite.position.set((x - 0.5) * (size / 2), y * (size / 12), z * (size / 4));
    ...
    onAnimation({ object3D: group }: SceneExport, { scene }: CanvasState) {
      group.children.forEach(child => {
        child.lookAt(scene?.getObjectByName('skull')?.position || new THREE.Vector3())
      })
    }
  } as unknown as Scene,
  floor: {
  ...
  skull: {
    ...
    onSetup({ object3D }: SceneExport) {
      gsap.timeline().to(object3D.position, { z: 20, y: 5, duration: 16 });
    },
  } as unknown as Scene,
  walls: {
    object: () =>
